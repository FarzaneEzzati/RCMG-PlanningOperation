    ...
    while terminate < 2:
        ...
        while True:  # while number 1
            ...
            for sense in ('upper', 'lower'):
                t_last += 1
                # Every time a splitting is going to occur, t_last is increased by 1.
                # If the split not feasible, still it counts as a node.
                temp_node = copy.copy(T1[t_bar])  # Copy the node selected for splitting
                temp_node.AddSplit(split_element, T1_x[t_bar][split_element], sense)  # Add the split to the new node
        ...
        for sp in SP[t[k]]:
            ...
            # This step is the convexification step. Input is x of t[k].
            # Output would be updated x for t[k] along with new disjunctive cuts added to the subproblems.
            # The subproblems now are ready to generate Benders' cut for the master problem.
