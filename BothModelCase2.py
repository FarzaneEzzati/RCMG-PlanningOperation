...
def MasterProb():
    ...
    master.update()
    master.write('Case2Models/Master.mps')
    with open('Case2Models/Master_X_Info.pkl', 'wb') as f:
...
def SubProb(scen):
    ...
    if True:
        ...
        L = {(i, t, g): (1 + (i - 1) * AG_scens[scen]) ** ReInvsYear * Load_ *  Load_scens[scen][g][t - 1]
    ...
    sub.update()
    sub.write(f'Case2Models/Sub{scen}.mps')
    ...
    with open(f'Case2Models/Sub{scen}-Tr.pkl', 'wb') as f:
...
if __name__ == "__main__":
    ...
    UB_dict = {1: {(1, 1): b * 300, (1, 2): b * 114, (1, 3): b * 100,
               ...
               2: {(1, 1): b *600, (1, 2): b *318, (1, 3): b *100,
                   (2, 1): b *600, (2, 2): b *299, (2, 3): b *100,
                   (3, 1): b *600, (3, 2): b *2142, (3, 3): b *100,
                   (4, 1): b *600, (4, 2): b *1190, (4, 3): b *100},

               3: {(1, 1): b *300, (1, 2): b *930, (1, 3): b *100,
                   (2, 1): b *300, (2, 2): b *40, (2, 3): b *100,
                   (3, 1): b *300, (3, 2): b *334, (3, 3): b *100}}
    ...
    if True:
        ...
        ES_gamma = 0.85
        DG_gamma = 0.95  # %
        DG_consumption = 0.3  # gal/kW
        FuelPrice = .80*3.61  # $/gal
        ...
        for scen in range(len(Probs)):
            SP[scen] = gp.read(f'Case2Models/Sub{scen}.mps', env=env2)
            with open(f'Case2Models/Sub{scen}-Tr.pkl', 'rb') as handle:
        ...
        master = gp.read('Case2Models/Master.mps', env=env2)
        ...
        if Report:
            ...
            for scen in SP.keys():
                    ...
                    for g in RNGMonth:
                        ...
                        for oh in outage_hours[:len(outage_hours) - 2]:
                            yll1 = SP[scen].getVarByName(f'Y_LL[{i},{oh},{g}]').x >= \
                                   0.75 * ((1 + (i - 1) * AG_scens[scen]) ** ReInvsYear) * Load_ *  Load_scens[scen][g][oh - 1]
                            yll2 = SP[scen].getVarByName(f'Y_LL[{i},{oh + 1},{g}]').x >= \
                                   0.75 * ((1 + (i - 1) * AG_scens[scen]) ** ReInvsYear) * Load_ * Load_scens[scen][g][oh]
                ...
                LOList.append(sum(((1 + (i - 1) * AG_scens[scen]) ** ReInvsYear) * Load_ * Load_scens[scen][g][t - 1]
                ...
                LnTList.append(sum(((1 + (i - 1) * AG_scens[scen]) ** ReInvsYear) * Load_ * Load_scens[scen][g][t - 1]
                                   for i in RNGSta for g in (1, 2, 3, 6, 7, 8, 9) for t in outage_hours if
                                   t in DontTrans[g]))
                ...
                LnOList.append(sum(((1 + (i - 1) * AG_scens[scen]) ** ReInvsYear) * Load_ * Load_scens[scen][g][t - 1]
                                   for i in RNGSta for t in RNGTime if t not in outage_hours for g in RNGMonth))
        ...
        pd.DataFrame(report, index=[0]).to_csv(f'Case2Models/Report-{Load_}.csv')
