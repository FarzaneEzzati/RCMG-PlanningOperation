...
def MasterProb():
    ...
    master.update()
    master.write('TempModels/Master.mps')
    with open('TempModels/Master_X_Info.pkl', 'wb') as f:
...
def SubProb(scen):
    ...
    sub.update()
    sub.write(f'TempModels/Sub{scen}.mps')
    ...
    with open(f'TempModels/Sub{scen}-Tr.pkl', 'wb') as f:
...
if __name__ == "__main__":
    ...
    for sns in [0.1]:
        ...
        for scen in range(len(Probs)):
            SP[scen] = gp.read(f'TempModels/Sub{scen}.mps', env=env2)
            with open(f'TempModels/Sub{scen}-Tr.pkl', 'rb') as handle:
        ...
        master = gp.read('TempModels/Master.mps', env=env2)
        ...
        pd.DataFrame(X1, index=[0]).to_csv('TempModels/X1.csv')  # master problem solution save
        pd.DataFrame(X2, index=[0]).to_csv('TempModels/X2.csv')  # subproblem solution save
        ...
        pd.DataFrame(report, index=[0]).to_csv(f'TempModels/Report-{sns}.csv')
